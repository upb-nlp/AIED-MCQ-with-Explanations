...
QG = QuestionGenerationUtils('cuda', 'ORGANIZATION-NAME/llama3.1_8b_qall_without_explanations', 'TOKEN_KEY')
...
for filename, data in tqdm(istart_dataset.items()):
    ...
    res = []
    for i in range(3):
        res += QG.generate_all_artifacts_without_explanations(context=context, num_samples=20)
...
json.dump(new_dataset, open("AIED-MCQ-with-Explanations/istart_questions_without_explanations.json", "w"), indent=4)
